...
// I am near to some location 
//if I am near it or the last action was skip (meaning that there are no paths to there)
/*+!near(X,Y) : (pos(AgX,AgY) & jia.neighbour(AgX,AgY,X,Y)) | last_dir(skip) 
   <- +near(X,Y).*/
   
/*Exercício B. Dividi near em dois para ter os prints */
// I am near to some location if I am near it 
+!near(X,Y) : (pos(AgX,AgY) & jia.neighbour(AgX,AgY,X,Y))
   <- 	.print("I am at (",AgX,",",AgY,"). Near of (",X,",",Y,")");
   		+near(X,Y).
//If the last action was skip, there are no paths to there
+!near(X,Y) : (pos(AgX,AgY) & last_dir(skip))
   <- 	.print("I am at (",AgX,",",AgY,"). I can't reach (",X,",",Y,")");
   		+near(X,Y).   
